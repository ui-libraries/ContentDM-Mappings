...
<!--    <!-\-   to tokenize collection guides -\->
    <xsl:template match="mods:relatedItem[mods:location]" exclude-result-prefixes="#all">
        <xsl:variable name="relatedItem-attributes" select="@*"/>
        <xsl:for-each select="mods:location/mods:url">
            <xsl:variable name="location-attributes" select="../@*"/>
            <xsl:variable name="url-siblings" select="*[not(self::mods:namePart)]"/>
            <xsl:for-each select="tokenize(.,';')">
                <relatedItem xmlns="http://www.loc.gov/mods/v3">
                    <location>
                        <xsl:copy-of select="$location-attributes"/>
                        <url>
                            <xsl:value-of select="normalize-space(.)"/>
                        </url>
                        <xsl:apply-templates select="$url-siblings"/>
                    </location>
                </relatedItem>
            </xsl:for-each>
        </xsl:for-each>
    ...
    <!--   to tokenize language-->
    <xsl:template match="mods:language[mods:languageTerm[contains(.,';')]]" exclude-result-prefixes="#all">
        <xsl:variable name="language-attributes" select="@*"/>
        <xsl:for-each select="mods:languageTerm">
            <xsl:for-each select="tokenize(.,';')">
                <language xmlns="http://www.loc.gov/mods/v3">
                    <xsl:copy-of select="$language-attributes"/>
    ...
    <xsl:template match="mods:mods/mods:originInfo[mods:publisher[normalize-space()]]" exclude-result-prefixes="#all">
        <xsl:variable name="publisher-attributes" select="@*"/>
        <!-- store sibling elements, e.g. dateCreated, dateCaptured, place, etc. -->
        <xsl:variable name="originInfo-siblings" select="*[not(self::mods:publisher)]"/>
        <xsl:for-each select="mods:publisher">
            <xsl:for-each select="tokenize(.,';')">
                <originInfo xmlns="http://www.loc.gov/mods/v3">
                    <xsl:copy-of select="$publisher-attributes"/>
                    <publisher><xsl:value-of select="normalize-space(.)"/></publisher>
                    <!-- each name should include the OriginInfo siblings -->
                    <xsl:apply-templates select="$originInfo-siblings"/>
                </originInfo>
            </xsl:for-each>
        </xsl:for-each>
    </xsl:template>
...
<!--    <!-\- geo parsing -\->
    ...
    <!-- target collections in the ui namespace -->
    <xsl:template match="mods:relatedItem[@otherType='islandoraCollection']/mods:identifier" exclude-result-prefixes="#all">
        <identifier xmlns="http://www.loc.gov/mods/v3"><xsl:value-of select="replace(.,'islandora:','ui:')"/></identifier>
    </xsl:template>
    
    <!-- conflate height and width? cm -->
